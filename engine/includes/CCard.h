...
const unsigned int CARDWIDTH	=	56;
const unsigned int CARDHEIGHT	=	78;
...
public:
	...
	bool Selected() const		{ return mSelected; }		//new addition
	...
	void Select()				{ mSelected = true; }	//new addition
	void Deselect()				{ mSelected = false; }	//new addition
...
// nudging, card highlighting substitute, also a new addition
	void NudgeToTop() {		//for left to right arrangement
		if(!mSelected)		{ y -= 5; Select(); }
  		else				{ y += 5; Deselect(); }
    }

	void NudgeToRight() {	//for top to bottom arrangement
		if(!mSelected)		{ x += 5; Select(); }
		else				{ x -= 5; Deselect(); }
	}
