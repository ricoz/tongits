...
//Used in my Tong-Its game
const unsigned int CRD_DONOTEMPTY	= ( 1 << 13 ); // Do not accept anything when stack is empty
...
class CCardRegion
{
	public:
		...
		void SelectCard(int Idx) {
  			if(!yOffset) InternalStack[Idx].NudgeToTop();
  			else if(!xOffset) InternalStack[Idx].NudgeToRight();
		}
		...
		//sets, gets the minimum no. of cards to accept if empty
		void SetMinCards(int min)						{ MinCards = min; }		//new addition
		int  GetMinCards()								{ return MinCards; }	//new addition

		void SetStackWidth(int width)					{ StackWidth = width; }
		void SetStackHeight(int height)					{ StackHeight = height; }
//---------------------------------------------------------------------------------------
//Defined in CCardRegion.cpp

		//new addition
		int GetNumSelectedCards();
		CCardStack GetSelectedCards();
		CCardStack PopSelectedCards();
...
//CCardStack object wrappers
		...
		int			FindCard(int pRank, int pSuit)		{ return InternalStack.FindCard(pRank, pSuit); }
		int			FindMoreSameRank(int pStart, int pRank, int pSuit)		{ return InternalStack.FindMoreSameRank(pStart, pRank, pSuit); }

		//new addition
		int			GetTotalRankValue()					{ return InternalStack.GetTotalRankValue(); }
//---------------------------------------------------------------------------------------

		int Id;
		int Idx; //new addition, the array index
	private:
