...
CCardRegion *CGame::CreateRegion(int id, unsigned int attribs, unsigned int amode, int dmode, int symbol, int x, int y, int xoffset, int yoffset)
{
...
{
	return InitDrag(NULL, NULL, x, y);
}

// optimization needed
bool CGame::InitDrag(CCardStack *CargoStack, CCardRegion *SRegion, int x, int y)
{
	if(SRegion != NULL) SourceRegion = SRegion;
...
//	dragface = SDL_CreateRGBSurface(SDL_SWSURFACE, dcard.width, dcard.height, 32, 0, 0, 0, 0);
    dragface = SDL_CreateRGBSurface(SDL_SWSURFACE, dcard.width, dcard.height, 24, 0, 0, 0, 0);
...
//new addition
void CGame::SelectCard(int x, int y)
{
	if(SourceRegion->Empty())
		return;
...
//	SDL_FillRect(screen,NULL,0x008040);
	...
	//Tong-Its
	SDL_BlitSurface(mGameBoard, NULL, screen, NULL);
