...
//new addition
int CCardStack::FindCard(int pRank, int pSuit) {
	int Idx = 0;

	for(VI vi = this->begin(); vi != this->end(); ++vi) {
		if(vi->Rank() == pRank && vi->Suit() == pSuit)
			return Idx;
		Idx++;
	}

	return -1; //no such card was found
}

int CCardStack::FindMoreSameRank(int pStart, int pRank, int pSuit) {
	int Idx = pStart;
	if(pStart > this->Size()) pStart = this->Size();
	for(VI vi = this->begin() + pStart; vi != this->end(); ++vi) {
		if(vi->Rank() == pRank && vi->Suit() != pSuit)
			return Idx;
		Idx++;
	}
...
//new addition, modify for 10=J=Q=K rules
int CCardStack::GetTotalRankValue() {
	int total = 0;
	for(VI vi = this->begin(); vi != this->end(); ++vi) {
		if(vi->Rank() > 10)
			total += 10;
